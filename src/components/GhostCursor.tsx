  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const comp = composerRef.current!;
        ...
        // Autonomous wandering ONLY
        const wanderX = 0.5 + Math.sin(t * 0.3) * 0.3 + Math.cos(t * 0.15) * 0.15;
        const wanderY = 0.5 + Math.cos(t * 0.2) * 0.3 + Math.sin(t * 0.1) * 0.15;
        
        velocityRef.current.set(
          wanderX - mat.uniforms.iMouse.value.x,
          wanderY - mat.uniforms.iMouse.value.y
        );
        mat.uniforms.iMouse.value.add(velocityRef.current.multiplyScalar(0.015)); // Make movement slower and smoother
        fadeOpacityRef.current = 0.4; // Force a low opacity for thin smoke
    ...
    // Completely ignore cursor events by NOT adding listeners
